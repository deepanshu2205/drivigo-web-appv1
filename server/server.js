...
const allowedOrigins = [
  'http://localhost:3000',
  'https://drivigo-web-appv1.vercel.app' // Removed trailing slash for safety
];
...
// --- Configurations using Environment Variables (Correctly) ---
...
app.get('/api/instructor/profile', authMiddleware, async (req, res) => {
  try {
    const profileData = await pool.query(
      `SELECT u.email, ip.photo_url, ip.car_model, ip.phone_number FROM users u
...
app.put('/api/instructor/profile', authMiddleware, async (req, res) => {
  try {
    const { car_model, photo_url, phone_number } = req.body;
    const userId = req.user.userId;
    const query = `
      INSERT INTO instructor_profiles (user_id, car_model, photo_url, phone_number)
      VALUES ($1, $2, $3, $4)
      ON CONFLICT (user_id)
      DO UPDATE SET car_model = EXCLUDED.car_model, photo_url = EXCLUDED.photo_url, phone_number = EXCLUDED.phone_number;
    `;
    await pool.query(query, [userId, car_model, photo_url, phone_number]);
