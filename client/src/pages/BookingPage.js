...
const MAPBOX_TOKEN = 'pk.eyJ1IjoibHVjaWZlci1kcml2aWdvIiwiYSI6ImNtZGV4ZzF5bDA2MWIyanM4Z2lrMWQ0cWIifQ.UBPdwEPM3z7GDIP2Mgalhw';
...
function BookingPage() {
  ...
  // Handles location change from the draggable map marker
  ...
  // Submits the search form to find instructors
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/instructors/find', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify(bookingDetails), // Now sends coordinates
      });

      if (!response.ok) {
        throw new Error('Failed to find instructors.');
      }
  ...
  // Handles selecting an instructor and initiating payment
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response) {
          // --- NEW: Step 1 ---
          // If we are already verifying, do nothing. This is our guard.
          ...
          try {
            ...
            // --- NEW: Step 3 ---
            // Always take the sign down when we're finished.
